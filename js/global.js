...
//for the comment fields
	$("input#author").attr("title", "Name *");

	$("input#email").attr("title", "Email *");
	
	$("input#url").attr("title", "Website");
	
	$("input#s").attr("title", "Search");
	
	$("textarea#comment").attr("title", "Comment");
	
	$('#commentform input, #commentform textarea, input#s').formtips({
			tippedClass: 'tipped'
	});
